...
const ModifySearchFlightsForm: React.FC<ModifySearchFlightsFormProps> = ({
  ...
  return (
    <div
      className={`absolute left-1/2 z-10 w-full -translate-x-1/2 transform rounded-b-lg bg-white pt-5 transition-all duration-500 md:w-[90%] lg:max-w-screen-lg ${
        show ? "-translate-y-0" : "z-0 -translate-y-20 opacity-0"
      }`}
    >
      <SearchFlightsForm />
    </div>
